...
export function WeeklyScheduleEditor({ doctorId, schedules }: Props) {
  ...
  const createMut = useMutationHandler(useCreateSchedule(doctorId), () => setOpen(false));
  const deleteMut = useMutationHandler(useDeleteSchedule(doctorId), () => setDeletingId(null));
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        {canEdit && (
          <Button size="sm" onClick={onOpenAdd}><Plus className="h-4 w-4 mr-2" /> Add Shift</Button>
        )}
      </CardHeader>
      <CardContent className="space-y-6 pt-4">
          ...
          return (
            <div key={dayIndex} className="border rounded-lg overflow-hidden">
              <div className="bg-muted/40 px-4 py-2 border-b font-medium text-sm flex items-center justify-between">
                <span>{dayName}</span>
                <Badge variant="outline" className="bg-background">{daySchedules.length} shift(s)</Badge>
              </div>
              <div className="divide-y bg-card">
                {daySchedules.map(shift => (
                  <div key={shift.id} className="p-4 flex flex-col md:flex-row md:items-start justify-between gap-4">
                    ...
                    {canEdit && (
                      <Button variant="ghost" size="sm" className="text-destructive shrink-0" onClick={() => setDeletingId(shift.id)}>
        ...
        <ModalForm
          ...
          description="Create a recurring block of working hours."
        ...
        >
          ...
          {createMut.error && <ErrorBubble err={createMut.error} />}
        </ModalForm>
        ...
        <ConfirmDialog
          ...
          title="Delete Shift"
          description="Are you sure you want to remove this shift? All associated breaks will be deleted."
...
// Minimal helpers local to the UI layer
function useMutationHandler(mut: any, onSuccess: () => void) {
  return {
    ...mut,
    mutate: (vars: any) => mut.mutate(vars, { onSuccess }),
  };
}

function ErrorBubble({ err }: { err: any }) {
  if (!err) return null;
  const status = err.response?.status;
  const msg = err.response?.data?.message || err.message;
  return (
    <div className="bg-destructive/10 border border-destructive/20 text-destructive text-sm rounded-md p-3">
      {status === 409 ? 'Time conflict detected.' : msg}
    </div>
  );
}
